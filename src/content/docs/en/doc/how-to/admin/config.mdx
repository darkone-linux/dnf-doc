...
Accounts common to all zones. Each account receives a [Home Manager](/en/doc/admin-guide/understand/user-profiles/) profile and groups.
...
| `email` | text | Address (also used for SSO identity), defaults to `<login>@<domain.tld>`. |
| `profile` | text | User profile (default `minimal`). |
| `groups` | list | Installs the user on hosts having the same group. |
...
:::tip[Groups = access]
A host authorizes a user if it lists them in `users`, **or** if one of their `groups` matches. See [Users](/en/doc/admin-guide/operate/users/).
:::
...
The `services` key contains a collection of services to be automatically installed on
the host. Values are optional and allow service customization.

:::note[Service type and node type]
Most services can be installed on any node, but some may have
constraints&nbsp;:

- Only one service per zone (e.g. adguardhome, ncps)
- Special out-of-zone service (e.g. headscale, turn)

Safeguards may prevent you from installing any
service anywhere.
:::
