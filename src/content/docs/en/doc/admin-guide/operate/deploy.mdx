...
## Update the fleet with fleet-update

`fleet-update` chains the whole update in one command&nbsp;: flakes, building every host, testing in waves (network-critical hosts first), then final activation of validated hosts.

:::danger[Not functional yet]
Only the interface exists&nbsp;: the command replays recorded scenarios (`just fleet-update --list`). Meanwhile, update with `just update-flake` then `just apply`, and leave `timer.enable` disabled.
:::
...
| `timer.extraArgs` | `[ "--send-report" ]` | Options added to `--no-ui` |
| `timer.timeout` | `6h` | Maximum duration of a run |

:::note[Exit codes and alerts]
A run already in progress (code `4`) is not an error. Any other failure, or a `timer.timeout` exceeded, puts the unit in a failed state and triggers the [alert](/en/doc/admin-guide/operate/alerts/) `SystemdUnitFailed`.
:::

:::caution[Account and deployment host]
The timer requires `darkone.admin.nix` on the host, and a `user` account with passwordless `sudo`.
:::
