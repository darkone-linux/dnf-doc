...
build of all hosts, staged testing (the hosts the network depends on first),
then definitive activation of the validated hosts.

:::caution[Options not yet available]
`--send-report` (Matrix report) is refused&nbsp;: error message and exit code
`2`. The `--ai-*` options are accepted, with no effect for now.
:::
...
| Step | What happens |
|---|---|
| Update | `nix flake update` of the project then of `dnf/` (co-development), `just clean`, one commit per repository |
| Selection | Hosts, profiles and zones read from `var/generated/`, filtered by `--on`; current zone deduced from the local IP |
| Build | All selected hosts, reachable or not, on the deployment machine |
| Test | `switch-to-configuration test`, one wave per profile; the next wave starts when all its hosts are OK or set aside |
| Switch | All hosts validated in test, **all at once**, regardless of wave order |
...
there is nothing left to protect&nbsp;: the switch goes out in one go, at most
`--max-parallel` hosts activated simultaneously. With `--skip-test`, nothing
has been proven&nbsp;: the switch follows the wave order again.
:::

:::tip[Resuming an interrupted launch]
`--resume` restarts from the last folder of `var/deployments/`&nbsp;: update
skipped, waves and options of the original launch, and the hosts that still
remained to do (remaining, offline, failed, or only tested). The builds are
reused if the revisions of the project and of `dnf/` have not moved and the
paths are still in the store&nbsp;; otherwise they are rebuilt. Nothing to
resume&nbsp;: exit code `2`, no launch.
:::
...
Two other optional blocks of `config.yaml` set the durations&nbsp;:
...
| `timeouts.<key>` | Maximum duration of an operation, in seconds | depending on the operation |

`timeouts` keys, in the order of the flow&nbsp;: `flakeUpdate` (120),
...
`just generate` checks the key names and refuses a zero or negative timeout&nbsp;:
a typo would otherwise silently fall back on the default value.
:::
...
| `timer.extraArgs` | `[ ]` | Options added to `--no-ui` |
| `timer.timeout` | `6h` | Maximum duration of a launch |

:::note[Exit codes and alerts]
A launch already in progress (code `4`) is not an error. Any other failure,
or an overrun of `timer.timeout`, puts the unit in failed state and triggers
the [alert](/en/doc/admin-guide/operate/alerts/) `SystemdUnitFailed`.
:::
