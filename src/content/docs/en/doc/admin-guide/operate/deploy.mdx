...
:::note[Activation]
`harmonia` and `ncps` are [services](/en/doc/admin-guide/operate/services/) to enable on a network host. The signing key is managed by [sops](/en/doc/admin-guide/operate/secrets/) (`just configure-admin-host`).
:::
