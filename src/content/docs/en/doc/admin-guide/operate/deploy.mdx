...
### Reports and Matrix alerts

Each run writes its folder to `var/deployments/<date>-<mode>/`:
`report.md` (the report), `state.json` (the state that `--resume` picks up),
`events.jsonl`, and per-host logs. The path is shown at the end of the run.
...
| **Diagnostics** | Per failing host: failing units, then error extract |
...
A host that pulls most of its content from `harmonia <zone>` proves that its
zone's cache does its job. A host that gets a lot of volume **pushed** to it is
on the contrary a candidate for a `harmonia` in its zone, or for `auto-build`.
No copy line for a host → it already held everything, nothing traveled.
:::

The `--send-report` option sends this report to the rooms of the
[alerts bot](/en/doc/admin-guide/operate/alerts/), installed beforehand by
`just configure-alert-bot`.

| Situation | Room | Content |
|---|---|---|
| Run finished | warnings | Summary: hosts by state, duration, known errors |
| Critical hosts not deployed | incidents | Separate message: concerned hosts, reasons, and failing units |
...
stop, the hosts left in `test` complete the incident message: a restart would
bring them back to their previous generation.

The message takes the report's header, the exit code in the title:
...
text. Nothing leaves if the bot isn't provisioned, nor on an abort (`^C`,
service stop).
:::
