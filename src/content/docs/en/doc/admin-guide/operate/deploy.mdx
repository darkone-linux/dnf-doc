...
Each run writes its folder to `var/deployments/<date>-<mode>/`: `report.md` (the report), `state.json` (the state that `--resume` resumes), `events.jsonl` and the per-host logs. The path is displayed at the end of the run.

The `--send-report` option sends this report to the channels of the [alerts bot](/en/doc/admin-guide/operate/alerts/), installed beforehand with `just configure-alert-bot`.

| Situation | Channel | Content |
|---|---|---|
| Finished run | warnings | Summary: hosts by state, duration, known errors |
...
The message uses the report's header, with the exit code in the title:
...
:::note[Impossible to send]
The run remains valid and the report is written, but the command exits with `3`.
Under the timer, the unit goes to a failed state, hence the `SystemdUnitFailed`
alert.
:::

:::caution[What sending requires]
Sending goes through the [`just send-msg`](/en/doc/admin-guide/operate/alerts/#sending-a-message-to-a-room) recipe: channels and token remain the framework's responsibility, the tool only writes the text. Nothing goes out if the bot is not provisioned, nor on an abort (`^C`, service stop).
:::
