...
`global` `harmonia`, then its zone's `nix-cache` proxy. The first retrieval from
the Internet is thus shared across the entire zone.

:::note[Activation]
`harmonia` and `nix-cache` are [services](/en/doc/admin-guide/operate/services/)
to activate on a network host (`nix-cache` on the zone's gateway). The harmonia
signing key is managed by [sops](/en/doc/admin-guide/operate/secrets/)
...
hence no HTTP 500 "invalid nar hash" error from the former `ncps` relay.
:::
