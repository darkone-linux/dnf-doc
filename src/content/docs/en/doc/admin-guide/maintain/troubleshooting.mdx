...
Publishing is refused if the staging area contains no files (`empty staging dir, refusing to publish`). Without this safeguard, synchronization would erase the certificates in place **and** the ACME account key. If this message appears, the issue is upstream: SSH access to the HCS, tailnet, or empty Caddy storage on the HCS side.
:::
