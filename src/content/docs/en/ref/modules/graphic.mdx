...
* **cursorSize** `ints.positive` Pointer size in pixels, locked host-wide. UMI hosts (`darkone.host.umi`) raise it to 48.
...
* **enable** `bool` QEMU test VMs bridged on the DNF LAN
* **bridge.name** `str` Bridge interface the VMs plug into.
* **bridge.interface** `str` Wired NIC enslaved into the bridge.
* **bridge.macAddress** `str` Bridge MAC address: the NIC's own, to keep the zone DHCP reservation.
