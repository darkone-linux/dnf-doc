...
* **enableNextcloud** `bool` Install the Nextcloud desktop client, pre-bound to the network instance
* **nextcloudServer** `nullOr str` Nextcloud instance the client binds to. `null` picks the one declared
  on the network (this zone first). Set it to target a local or external
  instance instead.
* **enableNextcloudAutoStart** `bool` Start the Nextcloud client in the background on login
* **enableNextcloudWebdav** `bool` Ship `nextcloud-webdav-login`, a one-shot helper that authenticates
...
* **nextcloudSyncDir** `nullOr str` Local folder pre-filled in the setup wizard, relative to `$HOME`.
