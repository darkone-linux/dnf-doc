...
* **nodeExpiry** `str` Key expiry of personal devices (`0`: never). Tagged machines never expire.
* **policy.enforce** `bool` Default-deny tailnet ACLs. `false` allows all traffic, for a migration only.
* **policy.adminDevices** `attrs` Personal devices granted SSH on every machine and zone: name -&gt; tailnet IPv4.
* **policy.exitNodeSources** `listOf str` Policy sources allowed to use the HCS as exit node.
* **policy.extraHosts** `attrs` Extra policy host aliases (name -&gt; CIDR), to use in `extraAcls`.
* **policy.extraAcls** `listOf (attrsOf anything)` ACL rules appended after the generated ones.
...
* **enable** `bool` Enable the local matrix administration UI
...
* **enable** `bool` Export the shared SuperTuxKart tracks (avoid enable manually)
* **enableClient** `bool` Mount the shared SuperTuxKart tracks of the zone
* **serverName** `nullOr str` Hostname serving the tracks, default is the zone's `stk` service host
...
* **enableSmtp** `bool` Enable SMTP to send emails (recommended, needs `network.smtp`)
