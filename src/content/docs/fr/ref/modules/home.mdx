...
{/* t:p b25824e111ae941fb46610eee12a50520c4e74b2fbbe97ae05c0affff2f6dc90 */}
### &#x1F3E0; darkone.home.office

Outils courants pour le bureau domestique.

* **enable** `bool` Packages utiles par défaut
...
* **enableProductivity** `bool` Apps de productivité (obsidian, gestion du temps, projets, etc.)
* **enableCommunication** `bool` Outils de communication
* **enableOffice** `bool` Suites bureautiques (libreoffice, huntspell, polices...)
* **enableFirefox** `bool` Activer firefox
* **enableLibreWolf** `bool` Activer LibreWolf (alternative à firefox)
* **enableChromium** `bool` Activer chromium
* **enableBrave** `bool` Activer Brave Browser
* **enableEmail** `bool` Paquets de gestion de messagerie (thunderbird)
* **enableSecurity** `bool` Outils de sécurité (keepass)
* **enableCalendarContacts** `bool` Calendrier et contacts Gnome, et apps associées
...
* **enableFractalAutoStart** `bool` Démarrage automatique de Fractal à la connexion lorsqu'un serveur Matrix local est présent (opt-in, voir les réserves ci-dessous)
* **enableNextcloud** `bool` Installer le client desktop Nextcloud, pré-lié à l'instance du réseau
* **nextcloudServer** `nullOr str` Instance Nextcloud à laquelle le client se lie. `null` sélection celle déclarée
  sur le réseau (cette zone en priorité). Remplacez-la pour cibler une instance locale ou externe.
* **enableNextcloudAutoStart** `bool` Démarrer le client Nextcloud en arrière-plan à la connexion
* **enableNextcloudWebdav** `bool` Fournit `nextcloud-webdav-login`, un assistant ponctuel qui s'authentifie
  via le navigateur (SSO Kanidm) et enregistre le résultat en tant que GNOME
  Online Account : fichiers via WebDAV dans le gestionnaire de fichiers, plus
  calendrier et contacts dans Evolution. Nécessaire car Nextcloud refuse de créer
  un mot de passe applicatif depuis son interface web pour un utilisateur OIDC.

  L'assistant affiche également le mot de passe applicatif, permettant à tout autre client WebDAV
  d'être pointé vers le même compte.

  :::note[Gnome uniquement]
  Le compte est écrit à l'endroit où GNOME Online Accounts le lit, ce que
  `darkone.graphic.gnome` active dès que le réseau
  contient un cloud. Sur un autre bureau, le mot de passe affiché reste utilisable.
  :::

  Ignoré quand `enableNextcloud` est désactivé.
* **nextcloudSyncDir** `nullOr str` Dossier local pré-rempli dans l'assistant de configuration, relatif à `$HOME`.

  :::caution[Pas de synchronisation par défaut]
  Laissé à `null` volontairement. L'assistant s'arrête alors sur sa page de dossier
  et l'utilisateur décide quoi — le cas échéant — synchroniser. Le définir saute
  cette page.
  :::
...
darkone.home.office = {
  ...
  enableNextcloud = detectedNextcloud != null;
  nextcloudServer = "https://cloud.example.com";
  enableNextcloudAutoStart = true;
  enableNextcloudWebdav = true;
  nextcloudSyncDir = "Nextcloud";
};
```
