...
{/* t:p 257000c70f8301eb389a3d38afad642c70dabdfb609861264c9b351737549679 */}
### &#x2728; darkone.service.harmonia

Harmonia : serveur local de cache binaire Nix (sert le /nix/store de cet hôte).

Expose les chemins de store construits / réalisés localement en HTTP simple
(port 5000), signés avec la clé de cache binaire du déploiement. Activable
par hôte depuis `usr/config.yaml` (services.harmonia), comme tout autre
service DNF.

:::note[Fonctionnement des services Harmonia avec NCPS]
Harmonia est une *source* de paquets ; `ncps` est le proxy de cache par zone
auquel les clients s'adressent réellement. Chaque serveur `ncps` de zone
ajoute automatiquement comme amont (upstreams) :

- les instances harmonia de *sa propre zone* (priorité la plus haute), puis
- toute harmonia marquée `global` (typiquement sur le HCS),

et jamais l'harmonia des *autres* zones (non globales). Les clients
récupèrent uniquement via `ncps` ; la signature d'harmonia est transmise sans
modification, donc chaque hôte fait confiance à la clé publique harmonia du
déploiement (`usr/secrets/harmonia.pub`). Le transport est en HTTP simple sur
le LAN/VPN de confiance : l'intégrité vient de la signature NAR, pas de TLS.
:::

:::tip[Provisionnement de la clé de signature]
`just configure-admin-host` génère automatiquement la paire de clés du
déploiement quand `usr/secrets/harmonia.pub` est absent : la clé publique est
versionnée et la clé privée est injectée dans le secret sops
`harmonia-secret-key`.
:::
...
ACL qui donne accès uniquement à l'utilisateur du service
(systemd/systemd#29435) ; harmonia vérifie naïvement les bits chmod. Surveillez
l'amont cependant : harmonia rejette déjà fermement les clés *TLS* lisibles par
le groupe au démarrage — si cette politique s'étend un jour aux clés de
signature, cette unité cassera à la mise à jour.
:::
...
{/* t:p 04083c3747f3e3710bb393f8500326bba123b94c6532d766a4154895627468dc */}
### &#x2728; darkone.service.matrix

Serveur matrix DNF (synapse) avec ponts mautrix.

Les ponts (whatsapp, signal, telegram, messenger, discord) sont utilisables
par tout compte local : chaque utilisateur relie son compte distant en
discutant avec le robot du pont (`@whatsappbot`, `@signalbot`, ... puis
`login`) ; les sessions sont isolées par utilisateur.
L'`network.matrix.admin` déclaré administre les ponts.
...
appservice `doublepuppet` partagé permet aux ponts d'envoyer les messages
provenant de services distants comme le compte matrix réel de l'utilisateur.
Secrets sops requis : `mautrix-doublepuppet-as-token` et
