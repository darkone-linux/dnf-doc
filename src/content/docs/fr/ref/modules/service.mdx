...
{/* t:p 67dac901f08a85ffae5b653be6cc1657c237010aa609ec58f8f35648e54c7bbd */}
### &#x2728; darkone.service.matrix

Serveur Matrix DNF (synapse) avec les ponts mautrix.

Les ponts (whatsapp, signal, telegram, messenger, discord) sont utilisables par tout compte local&nbsp;: chaque utilisateur relie son propre compte distant en dialoguant avec le robot du pont (`@whatsappbot`, `@signalbot`, ... puis `login`)&nbsp;; les sessions sont isolées par utilisateur. Le `network.matrix.admin` déclaré administre les ponts.
...
- Alertmanager achemine par sévérité&nbsp;: warning → Matrix #warnings,
  critical → Matrix #incidents + mail (relais Postfix local).
- blackbox_exporter sonde la reachabilité gateway / tailnet / DNS.
- Les classes de nœuds (critique/non-critique/disabled) proviennent des
  features `alert-*` ou du profil hôte (voir `dnf/lib/alerts.nix`).
...
* **enable** `bool` Activer le serveur de métriques + alerting Prometheus
* **retentionTime** `str` Durée de rétention des métriques Prometheus
* **alerting.enable** `bool` Activer Alertmanager (activé par défaut une fois les salons d'alerte Matrix provisionnés)
* **alerting.matrix.enable** `bool` Envoyer les alertes dans des salons Matrix via le bot matrix-alertmanager
* **alerting.matrix.userId** `str` Identifiant Matrix du bot d'alertes (défaut depuis network.matrix.bot)
* **alerting.matrix.warningsRoom** `str` Identifiant du salon Matrix pour les avertissements (défaut depuis network.matrix.warningsRoom)
* **alerting.matrix.incidentsRoom** `str` Identifiant du salon Matrix pour les incidents (défaut depuis network.matrix.incidentsRoom)
* **alerting.email.enable** `bool` Envoyer aussi les alertes critiques par courriel (via le relais Postfix local)
* **alerting.email.to** `str` Destinataire des courriels d'alerte critique
* **alerting.network.enable** `bool` Sonder la reachabilité réseau (gateway/tailnet/DNS) avec blackbox_exporter
* **alerting.silenceOnRebuild** `bool` Envoyer le flag dnf-maintenance (collecteur textfile) pour que les rebuilds inhibent les alertes d'un nœud
* **alerting.thresholds** `attrs` Surcharger les seuils d'alerte ressource (voir défauts dans dnf/lib/alerts.nix)
