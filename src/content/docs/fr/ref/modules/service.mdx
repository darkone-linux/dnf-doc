...
{/* t:p bd24403f81cddc364441e7053690975aca2f1497d9db5f6c725a323e368a5ec5 */}
...
:::danger[Immuable après le premier démarrage]
`mas-encryption-secret` et l'ULID du fournisseur Kanidm ne doivent jamais
changer après le premier démarrage de MAS (données chiffrées en base / liens
de comptes amont).
:::

:::caution[Migration d'une instance existante]
Activer `mas.enable` sur un serveur déjà en production nécessite la migration
`mas-cli syn2mas` (comptes, sessions, identifiants externes) et une
régénération des enregistrements de ponts (le flag `io.element.msc4190` n'est
écrit qu'au moment de la génération)&nbsp;: procédure dans
...
Quand ce module est activé, il lit le mot de passe administrateur Nextcloud
depuis le secret sops `nextcloud-admin-password`. Ajoutez l'entrée dans
...
* **plugins** `listOf (enum (appstoreApps ++ shippedToggleableApps))` Applications Nextcloud à activer. Seules `calendar` et `contacts` sont
activées par défaut&nbsp;; Talk (`spreed`), le tableau de bord, le flux
d'activité, les photos et toute autre application restent désactivées
tant qu'elles ne sont pas listées ici.
...
{/* t:p 4cc2c30044225948c46c2f0c744d31d42df99400fc4c97f341251bffb163c980 */}
...
Un point de terminaison de cache fait face à exactement **une** source
amont. Un `narinfo` et son `nar` sont deux requêtes distinctes et l'URL du
`nar` est indépendante de la source amont, donc un proxy multi-sources doit
réécrire les URLs `narinfo` dans son propre espace de noms et les suivre dans
une base de données (l'approche qu'avait l'ancien proxy `ncps` — et la source
de ses HTTP 500 récurrents « hash nar invalide »). Avec une source unique, il
n'y a pas d'ambiguïté :
nginx met en cache `narinfo` + `nar` verbatim, les signatures passent
inchangées et les clients les vérifient. Pas de réécriture, pas de base de
données, pas de tel bogue par construction.
:::

:::tip[Topologie]
- serveur : l'hôte passerelle qui liste `nix-cache` dans `config.yaml` (un
  par zone). Fait tourner nginx sur l'IP interne, port `network.ports.nixCache`.
- clients : tous les hôtes de la zone locale. Leurs substituters deviennent
  les instances harmonia de la zone (direct, LAN/tailnet) plus ce proxy.
- clients nomades (`roaming = true`) : hôtes nomades déplacés entre zones.
  ...
  services de cache (voir `service/dnsmasq.nix`), ainsi le cache suit le
  portable.
:::
...
* **roaming** `bool` Hôte nomade (portable déplacé entre zones) : remplace les
substituters liés à la zone par les noms neutres de zone que le DNS de chaque
zone résout vers ses propres services de cache. En dehors de toute zone DNF,
les noms deviennent instantanément NXDOMAIN et Nix se rabat sur
cache.nixos.org.
