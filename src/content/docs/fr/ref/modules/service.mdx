...
{/* t:p 17abbc3ff9e07de1cba39178193feb6a09e29da31648bab4a8ee4484644d763a */}
### &#x2728; darkone.service.restic

Module de sauvegarde Restic avec configuration DNF.

:::tip
Activer « restic » dans les services de l'hôte (config.yaml) lance le serveur REST.
Placez les paramètres de sauvegarde locale dans la configuration nix (machine conf par exemple).
...
* **enableSystemBackup** `bool` Activer la sauvegarde système complète sauf /srv, /mnt et les fichiers cache
* **enableExtraSystemBackup** `bool` Activer la sauvegarde système sur un dépôt supplémentaire
* **mainBackupZone** `str` Zone de la sauvegarde principale (pour sélectionner le bon mot de passe)
* **extraBackupZone** `str` Zone de la sauvegarde supplémentaire (pour sélectionner le bon mot de passe)
...
* **enableNfsBackup** `bool` Sauvegarder les répertoires /srv/nfs/&lt;xxx&gt;
...
* **enableMediasBackup** `bool` Sauvegarder les répertoires /srv/medias/&lt;xxx&gt;
* **enableExtraMediasBackup** `bool` Activer la sauvegarde des médias sur un dépôt supplémentaire
...
darkone.service.restic = {
  ...
  extraBackupZone = "";
  repositoryRoot = "rest:restic.${zone.domain}:${toString srvPort}";
  extraRepositoryRoot = "rest:restic.${network.zones.other-zone.domain}:${toString srvPort}";
...
Un moteur de recherche renforcé entièrement configuré.
...
Ne pas l'utiliser pour connecter une passerelle vers un sous-réseau tailnet.
:::
