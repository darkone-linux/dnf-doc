...
* **enable** `bool` Active le tableau de bord de la page d'accueil + httpd + hôte
* **protect** `bool` Restreint la page d'accueil aux utilisateurs Kanidm authentifiés (oauth2-proxy forward auth)
* **protectExternalOnly** `bool` Quand elle est protégée, laisse les clients LAN/tailnet internes entrer sans connexion ; authentifie seulement les clients externes
...
* **bookmarks** `listOf attrs` Remplace les marque-pages par défaut (liens)
* **widgets** `listOf attrs` Remplace les widgets par défaut
...
{/* t:p a4820ace01564f46d58b9a4d694e783fa9e705e94cedb8d8d1a1bf5a98c3b4f4 */}
...
* **selfHeal.enable** `bool` Watchdog&nbsp;: détecter la déconnexion de headscale et redémarrer tailscaled.
