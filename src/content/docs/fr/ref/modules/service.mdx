...
{/* t:p 3fa8b3c185199ec6a6383bc2099bd48b0ef6ba5f71a86fd627a8d2d9130f032c */}
### &#x2728; darkone.service.prometheus

Serveur de supervision Prometheus avec escalade Alertmanager déclarative.

:::tip
Service autonome de métriques + alerting, indépendant de Grafana :

- Scrute chaque nœud de zone portant la fonctionnalité `monitoring-node:<zone>`.
- Alertmanager achemine par sévérité : warning → Matrix #warnings,
  critique → Matrix #incidents + courriel (relais Postfix local).
- blackbox_exporter sonde la joignabilité de la passerelle / tailnet / DNS.
- Les classes de nœuds (critique/non-critique/désactivé) proviennent des fonctionnalités
  `alert-*` ou du profil hôte (voir `dnf/lib/alerts.nix`).

`monitoring` (Grafana) nécessite un `prometheus` sur le même hôte
(imposé par `require` du générateur). L'interface web est exposée sur
`https://prometheus.<zone>.<domain>` derrière oauth2-proxy (groupe `admins`),
qui corrige également les liens d'alerte pointant vers Prometheus.
::~

But : utiliser, maintenir, configurer, déboguer.
...
* **alerting.enable** `bool` Activer Alertmanager (activé par défaut une fois les salles d'alerte Matrix approvisionnées)
* **alerting.matrix.enable** `bool` Envoyer les alertes dans les salons Matrix via le bot matrix-alertmanager
* **alerting.matrix.userId** `str` Identifiant Matrix du bot d'alerte (par défaut depuis network.matrix.bot)
* **alerting.matrix.warningsRoom** `str` Identifiant du salon Matrix pour les avertissements (par défaut depuis network.matrix.warningsRoom)
* **alerting.matrix.incidentsRoom** `str` Identifiant du salon Matrix pour les incidents (par défaut depuis network.matrix.incidentsRoom)
...
* **alerting.network.enable** `bool` Sonder la joignabilité réseau (passerelle/tailnet/DNS) avec blackbox_exporter
* **alerting.silenceOnRebuild** `bool` Transmettre le indicateur dnf-maintenance (collecteur textfile) pour que les reconstructions inhibent les alertes d'un nœud
* **alerting.thresholds** `attrs` Surcharger les seuils d'alerte de ressources (voir valeurs par défaut dans dnf/lib/alerts.nix)
