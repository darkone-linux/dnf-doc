...
(`just fleet-update --list`). En attendant, mettre à jour avec
`just update-flake` puis `just apply`, et laisser `timer.enable` désactivé.
:::
...
| `timer.extraArgs` | `[ "--send-report" ]` | Options ajoutées à `--no-ui` |
