...
* **enableDex** `bool` Enable Dex service
...
darkone.host.minimal = {
  ...
  enableDex = builtins.hasAttr "dex" host.services;
...
* **enable** `bool` Enable local SSO with Authelia
* **enableLdapBackend** `bool` Enable the LLDAP backend with Authelia

```nix
darkone.service.auth = {
  enable = false;
  enableLdapBackend = false;
};
```
...
* **enable** `bool` Enable dex service
