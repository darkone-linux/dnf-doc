...
* **enableSSO** `bool` Enable SSO service (LLDAP + Authelia + OIDC)
```nix
darkone.host.gateway = {
  enable = false;
  enableFail2ban = true;
  ...
  enableSSO = builtins.hasAttr "sso" host.services;
}
...
* **authDomainName** `str` Domain name for authentication (SSO), registered in network configuration
* **usersDomainName** `str` Domain name for user management (SSO), registered in network configuration
```nix
darkone.service.sso = {
  enable = false;
  authDomainName = "auth";
  usersDomainName = "users";
}
