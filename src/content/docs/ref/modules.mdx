...
* **systemDirsPaths** `listOf str` System dirs (exports) to include

```nix
darkone.service.borg = {
  enable = false;
  enableDryRun = false;
  repositoryRoot = "/mnt/backup/borg";
...
```nix
darkone.service.navidrome.enable = false;
```
...
* **darkone.service.nfs.enable** `bool` Enable NFS DNF server (avoid enable manually)
* **darkone.service.nfs.serverDomain** `str` NFS Server FQDN

```nix
darkone.service.nfs = {
  darkone.service.nfs.enable = hasServer && (isServer || isClient);
  darkone.service.nfs.serverDomain = "nfs";
};
```
...
* **repositoryRoot** `str` Main backup target root path
* **enableServicesBackup** `bool` Backup important files of local services (persist dirs)
* **enableServicesVarBackup** `bool` Backup variable files of local services (persist varDirs)
...
darkone.service.restic = {
  ...
  enableDryRun = false;
  enableWaitRemoteFs = true;
  repositoryRoot = "rest:nix@backup.${zone.domain}:/mnt/backup/restic/${host.hostname}";
  enableServicesBackup = true;
  enableServicesVarBackup = true;
  enableNfsBackup = config.darkone.service.nfs.enable;
  nfsPaths = [ ];
  enableMediasBackup = config.darkone.service.jellyfin.enable;
  mediasPaths = [ ];
};
```
...
* **darkone.system.core.enable** `bool` Darkone framework core system (activated by default)
* **darkone.system.core.enableSystemdBoot** `bool` Enable the default boot loader
* **darkone.system.core.enableFstrim** `bool` SSD optimisation with fstrim
* **darkone.system.core.enableFirewall** `bool` Enable firewall (default true)
* **darkone.system.core.enableSops** `bool` Enable sops dnf module (default true)
* **darkone.system.core.enableFlatpak** `bool` Enable flatpak DNF configuration (only for graphic environments)
...
```nix
darkone.system.core = {
  darkone.system.core.enable = true;
  darkone.system.core.enableSystemdBoot = true;
  darkone.system.core.enableFstrim = true;
  darkone.system.core.enableFirewall = true;
  darkone.system.core.enableSops = true;
  darkone.system.core.enableFlatpak = true;
  enableBoost = false;
...
* **darkone.system.srv-dirs.enable** `bool` Enable srv dirs, create the root dir (default /srv)
...
```nix
darkone.system.srv-dirs = {
  darkone.system.srv-dirs.enable = cfg.enableNfs || cfg.enableMedias;
  enableNfs = false;
  enableMedias = false;
  root = "/srv";
  nfs = ;
...
* **darkone.user.build.enable** `bool` Users common builder (enabled by default)

```nix
darkone.user.build.darkone.user.build.enable = true;
```
...
* **darkone.user.root.enable** `bool` Root user specific configuration

```nix
darkone.user.root.darkone.user.root.enable = true;
```
