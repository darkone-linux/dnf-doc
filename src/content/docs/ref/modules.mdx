...
* **domainName** `str` Domain name for Adguard Home, registered in global network
```nix
darkone.service.adguardhome = {
  enable = false;
...
* **domainName** `str` Domain name for authentication (SSO), registered in network configuration
```nix
darkone.service.auth = {
  enable = false;
  domainName = "auth";
...
* **domainName** `str` Domain name for the forge, registered in network configuration
...
* **domainName** `str` Domain name for immich, registered in local network
...
* **domainName** `str` Domain name for matrix web client, registered in network configuration
* **appName** `str` Default title for matrix service
```nix
darkone.service.matrix = {
  enable = false;
  domainName = "matrix";
...
* **domainName** `str` Domain name for mattermost, registered in network configuration
* **appName** `str` Default title for mattermost service
```nix
darkone.service.mattermost = {
  enable = false;
  domainName = "mattermost";
  appName = "Mattermost";
}
...
* **domainName** `str` Domain name for monitoring, registered in local network
...
* **domainName** `str` Domain name for netdata, registered in local network
```nix
darkone.service.netdata = {
  enable = false;
  domainName = "netdata";
...
* **domainName** `str` Domain name for nextcloud, registered in network configuration
...
* **domainName** `str` Domain name for sync server, registered in network configuration
...
darkone.service.syncthing = {
  ...
  ldapServerHost = "${network.gateway.hostname}.${network.domain}";
  ldapServerPort = "3890";
}
...
* **domainName** `str` Domain name for user management (SSO), registered in network configuration
```nix
darkone.service.users = {
  enable = false;
  domainName = "users";
...
* **enableObsidian** `bool` Obsidian application
